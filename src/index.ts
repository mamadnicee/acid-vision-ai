...
<div class="mobile-frame">
  <header>
    <div class="logo">ACID-VISION // v12.0</div>
    ...
    <div class="view-section active" id="viewUpscale">
      ...
      <div class="collapsible-field" id="fieldSDXL">
        <div class="field-header sdxl-variant" id="fieldSDXLHeader">
          <span class="f-icon">🎨</span>
          <div class="f-title">FLUX SCHNELL ENGINE — HD RENDER</div>
          <div class="f-sub">FLUX.1 Schnell on Cloudflare GPU · steps: 8 · max quality</div>
          <div class="f-chevron">▼</div>
        </div>
        <div class="field-body">
          <div class="control-group">
            <div class="control-label">FLUX Prompt</div>
          ...
          <div class="control-group">
            <div class="control-label">FLUX Output Format</div>
          ...
          <div class="control-group">
            <div class="control-label">FLUX Seed</div>
            <div class="mini-row">
          ...
          <div class="enhance-toggle" id="sdxlEnhanceToggle">
            <div>
              <div class="lbl">⚡ FLUX PROMPT ENHANCER</div>
              <div class="sub">injects masterpiece + HD keywords</div>
            </div>
            <div class="switch on" id="sdxlEnhanceSwitch"></div>
          </div>
          <button class="cyber-btn" id="sdxlGenerateBtn">⚡ GENERATE WITH FLUX HD</button>
          <div class="generation-output-box" id="sdxlOutputBox" style="min-height:240px;">
            <div class="placeholder-text" id="sdxlPlaceholder">READY FOR FLUX HD RENDER…<br><span style="color:#334155;font-size:.62rem">Enter a prompt · generate with Cloudflare FLUX.1 Schnell.</span></div>
            <img id="sdxlGeneratedImg" style="display:none;" alt="FLUX Artwork">
          </div>
          <div class="terminal" id="sdxlLogs" style="height:100px;"></div>
          <button class="cyber-btn secondary" id="sdxlDownloadBtn" style="display:none;">DOWNLOAD FLUX HD IMAGE</button>
        </div>
      </div>
    </div>
  ...
  <footer>ACID-VISION v12.0 // CLOUDFLARE FLUX.1 + MULTI-MODEL</footer>
</div>
...
wireOptions('engineOptions','engine');wireOptions('filterOptions','filter');wireOptions('aspectOptions','aspect');wireOptions('aiEngineOptions','aiEngine');wireOptions('sdxlAspectOptions','sdxlAspect');
...
el.sdxlGenerateBtn.addEventListener('click',()=>{
  ...
  setStatus('FLUX RENDERING…');el.sdxlGenerateBtn.disabled=true;
  el.sdxlGeneratedImg.style.display='none';el.sdxlDownloadBtn.style.display='none';
  el.sdxlPlaceholder.style.display='block';
  el.sdxlPlaceholder.innerHTML='CONNECTING TO CLOUDFLARE FLUX.1 SCHNELL…<br><span style="color:#334155;font-size:.62rem">HD mode · steps: 8 · GPU-accelerated</span>';

  const A=analyze(raw);const detected=A.tags.filter(t=>t!=='color').length;
  logLine(el.sdxlLogs,'→ Dispatching to Cloudflare FLUX.1 schnell (HD)…');
  ...
  logLine(el.sdxlLogs,'Steps: 8 (maximum for Schnell).');
  logLine(el.sdxlLogs,'Endpoint: '+location.origin);

  const t0=performance.now();
  runLoader('CLOUDFLARE FLUX.1 SCHNELL HD — RENDERING…',800,(done)=>{
    generateWithCloudflare(finalPrompt)
    ...
    if(res.ok){
      ...
      logLine(el.sdxlLogs,'Engine: Cloudflare FLUX.1 schnell HD.');
      logLine(el.sdxlLogs,'Output buffered. Ready to download.');
      setStatus('FLUX COMPLETE');
    } else {
      ...
      setStatus('FLUX FALLBACK');
    }
  });
});
el.sdxlDownloadBtn.addEventListener('click',()=>{const a=document.createElement('a');a.download='acid-vision-flux-'+state.sdxlVariant+'.png';if(state.sdxlGeneratedURL){a.href=state.sdxlGeneratedURL;a.click();}});
...
refreshChips();
logLine(el.genLogs,'AI Diffusion Engine v12.0 online.');
logLine(el.genLogs,'6 engines loaded via Pollinations.');
logLine(el.sdxlLogs,'FLUX HD Engine — Cloudflare FLUX.1 Schnell online (steps: 8).');
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      // فقط FLUX.1 Schnell با steps: 8 برای بهترین کیفیت
