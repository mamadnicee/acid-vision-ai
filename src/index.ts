...
document.getElementById("b").onclick=async()=>{
  ...
  try{
    ...
    body:JSON.stringify({prompt:"a cyberpunk samurai in neon Tokyo at night, cinematic, hyper detailed",width:1024,height:1024})});
    const d=await r.json();const sec=((Date.now()-t0)/1000).toFixed(1);
    if(d.ok&&d.image){s.textContent="✅ موفق در "+sec+"s | Seed: "+d.seed;
...
export default {
  ...
  const response = await env.AI.run("@cf/black-forest-labs/flux-1-schnell" as any, {
    prompt: prompt.slice(0, 2048),   // سقف ۲۰۴۸ کاراکتر
    steps: Math.min(Math.max(body.steps || 4, 1), 8)  // بین ۱ تا ۸
  });
