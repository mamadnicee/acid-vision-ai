...
.field-header.sdxl-header-style:hover{background:rgba(255,0,127,.10);}
...
.field-header.sdxl-variant{border-top:1px solid rgba(255,0,127,.25);}
    ...
    <div class="view-section active" id="viewUpscale">
      ...
      <div class="collapsible-field" id="fieldSDXL">
        <div class="field-header sdxl-variant" id="fieldSDXLHeader">
          ...
          <div class="f-sub">FLUX.1 on Cloudflare GPU · real AI render</div>
          <div class="f-chevron">▼</div>
        </div>
        <div class="field-body">
          <div class="control-group">
...
function compile(list){return list.map(e=>{const name=e[0];const words=e[1].map(w=>{if(/[a-z]/i.test(w)){const esc=w.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');return new RegExp('(^|[^a-z0-9])'+esc+'([^a-z0-9]|$)','i');}return w;});return [name,words];});}
...
const state={engine:'stable',filter:'Normal',aspect:'1:1',aiEngine:'flux',enhance:true,variant:0,sdxlAspect:'1:1',sdxlEnhance:true,sdxlVariant:0,sdxlGeneratedURL:null,sourceImg:null,enhancedCanvas:null,generatedURL:null,generatedCanvas:null};
...
el.sdxlGenerateBtn.addEventListener('click',()=>{
  ...
  el.sdxlPlaceholder.innerHTML='CONNECTING TO CLOUDFLARE FLUX.1…<br><span style="color:#334155;font-size:.62rem">GPU-accelerated render · usually 2–6 seconds</span>';

  const A=analyze(raw);const detected=A.tags.filter(t=>t!=='color').length;
  logLine(el.sdxlLogs,'→ Dispatching to Cloudflare FLUX.1…');
  ...
  const t0=performance.now();
  runLoader('CLOUDFLARE FLUX.1 — RENDERING…',800,(done)=>{
...
el.sdxlDownloadBtn.addEventListener('click',()=>{const a=document.createElement('a');a.download='acid-vision-cloudflare-'+state.sdxlVariant+'.png';if(state.sdxlGeneratedURL){a.href=state.sdxlGeneratedURL;a.click();}});
...
logLine(el.sdxlLogs,'SDXL HD Engine v11.5 — Cloudflare FLUX.1 online (same origin).');
